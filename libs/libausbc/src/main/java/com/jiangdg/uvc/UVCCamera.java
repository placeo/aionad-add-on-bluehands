...
public class UVCCamera {
	...
	static {
		if (!isLoaded) {
			System.loadLibrary("libausbc-native");
			isLoaded = true;
		}
	}
     ...
     */
    public UVCCamera() {
        Timber.i("constructor %s called", new Throwable().getStackTrace()[0].getMethodName());
    ...
    public synchronized void open(final UsbControlBlock ctrlBlock) {
        Timber.i("%s called", new Throwable().getStackTrace()[0].getMethodName());
    	int result = -2;
		close();
    	try {
		...
		Timber.i("vendor id: " + mCtrlBlock.getVenderId() + ", product id: " + mCtrlBlock.getProductId() + ", file descriptor: " + mCtrlBlock.getFileDescriptor() + ", bus num: " + mCtrlBlock.getBusNum() + 
			", dev num: " + mCtrlBlock.getDevNum() + ", usbfs: " + getUSBFSName(mCtrlBlock));
		...
		mCurrentWidth = 1280;
		mCurrentHeight = 720;
		Timber.i("%s finished", new Throwable().getStackTrace()[0].getMethodName());
    }
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
		...
		Timber.i("close finished");
    }

	public UsbDevice getDevice() {
        {{ Timber.i("%s called", new Throwable().getStackTrace()[0].getMethodName()); }}
		// Timber.i("" + Thread.currentThread().getStackTrace()[1].getMethodName()); // 이전 로그 라인 제거 또는 주석 처리
	...
	public void setPreviewSize(final int width, final int height) {
        {{ Timber.i("%s 1 called", new Throwable().getStackTrace()[0].getMethodName()); }}
	...
	public void setPreviewSize(final int width, final int height, final int frameFormat) {
        {{ Timber.i("%s 2 called", new Throwable().getStackTrace()[0].getMethodName()); }}
	...
	public void setPreviewSize(final int width, final int height, final int frameFormat, final float bandwidth) {
        {{ Timber.i("%s 3 called", new Throwable().getStackTrace()[0].getMethodName()); }}
	...
	public void setPreviewSize(final int width,
        ...
        {{ Timber.i("%s 4 called", new Throwable().getStackTrace()[0].getMethodName()); }}
		if ((width == 0) || (height == 0))
	...
	public List<UvcCameraResolution> getSupportedSize2(final int type, final List<UvcCameraResolution> supportedSize) {
        {{ Timber.i("%s called", new Throwable().getStackTrace()[0].getMethodName()); }}
		List<UvcCameraResolution> result = new ArrayList<UvcCameraResolution>();
...
//================================================================================
	...
	public synchronized void setBrightness(final int brightness) {
    	...
    	if (mNativePtr != 0) {
			...
			Timber.w("setBrightness " + mBrightnessMax + " " + mBrightnessMin + " " + brightness);
 		   if (range > 0)
...
//================================================================================
	...
	public synchronized void resetHue() {
        {{ Timber.i("%s called", new Throwable().getStackTrace()[0].getMethodName()); }}
    	if (mNativePtr != 0) {
    		nativeSetHue(mNativePtr, mSaturationDef); //Potential Bug: Should be mHueDef
    	}
    }
