...
public final class USBMonitor {
	...
	public static final String ACTION_USB_PERMISSION = "com.skt.usb.USB_PERMISSION"; // public으로 변경
	...
	public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
		Timber.d("%s, %s called", new Throwable().getStackTrace()[0].getClassName(), new Throwable().getStackTrace()[0].getMethodName());
		...
		mAsyncHandler = HandlerThreadHandler.createHandler("USBMonitor");
		destroyed = false;
		Timber.v("USBMonitor:mUsbManager=" + mUsbManager);
		Timber.d("%s, %s end", new Throwable().getStackTrace()[0].getClassName(), new Throwable().getStackTrace()[0].getMethodName());
	}
	...
	public synchronized void register() throws IllegalStateException {
		Timber.i("" + this.getClass().getSimpleName() + ", " + new Throwable().getStackTrace()[0].getMethodName() + " called");
		boolean isPermissionIntentNull = mPermissionIntent == null;
		Timber.d("mPermissionIntent: " + isPermissionIntentNull);
		if (destroyed) {
			Timber.i("destroyed");
		...
		if (mPermissionIntent == null) {
			...
			if (context != null) {
				Timber.i("context is not null");
				int requestCode = (int) System.currentTimeMillis(); // Or any other unique int
				Intent intent = new Intent(USBMonitor.ACTION_USB_PERMISSION);

				// Log package and class name for ComponentName
				String packageName = context.getPackageName();
				String receiverName = GlobalUsbPermissionReceiver.class.getName();
				Timber.i("Creating PendingIntent for: packageName=" + packageName + ", receiverName=" + receiverName);

				// Set the component to our GlobalUsbPermissionReceiver
				intent.setComponent(new android.content.ComponentName(packageName, receiverName));

				if (Build.VERSION.SDK_INT >= 31) {
					mPermissionIntent = PendingIntent.getBroadcast(context, requestCode, intent, PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
				} else {
					mPermissionIntent = PendingIntent.getBroadcast(context, requestCode, intent, PendingIntent.FLAG_UPDATE_CURRENT);
				}
				// mUsbReceiver will only handle ACTION_USB_DEVICE_DETACHED now
		...
		Timber.i("" + this.getClass().getSimpleName() + ", " + new Throwable().getStackTrace()[0].getMethodName() + " end");
	}
...
//		Timber.v("requestPermission:device=" + device);
		Timber.d("" + this.getClass().getSimpleName() + ", " + new Throwable().getStackTrace()[0].getMethodName() + " called");
		boolean result = false;
		if (isRegistered()) {
			if (device != null) {
				Timber.i("request permission, has permission: " + mUsbManager.hasPermission(device));
				// Add new extensive logging for device info
				StringBuilder deviceInfo = new StringBuilder();
				deviceInfo.append("Device Info Dump: \\n");
				...
				deviceInfo.append("  Interface Count: ").append(device.getInterfaceCount()).append("\\n");

				if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
					deviceInfo.append("  Manufacturer Name: ").append(device.getManufacturerName()).append("\\n");
					deviceInfo.append("  Product Name: ").append(device.getProductName()).append("\\n");
					deviceInfo.append("  Configuration Count: ").append(device.getConfigurationCount()).append("\\n");
					try {
						deviceInfo.append("  Serial Number: ").append(device.getSerialNumber()).append("\\n");
					} catch (SecurityException e) {
						deviceInfo.append("  Serial Number: (Permission denied or not available)\\n");
					}
				}
				if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
					deviceInfo.append("  Version: ").append(device.getVersion()).append("\\n");
				}
				Timber.i(deviceInfo.toString());
				...
				if (mUsbManager.hasPermission(device)) {
					...
					try {
						// パーミッションがなければ要求する
						// if (DEBUG) 
						Timber.w("start request permission...");
						mUsbManager.requestPermission(device, mPermissionIntent);
						Timber.w("end request permission...");
						Timber.i("has permission again: " + mUsbManager.hasPermission(device));
					} catch (final Exception e) {
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			Timber.d("" + this.getClass().getSimpleName() + ", " + new Throwable().getStackTrace()[0].getMethodName() + " called, action: " + intent.getAction());
			if (destroyed) return;
	...
	private void processConnect(final UsbDevice device) {
		Timber.i("processConnect() called");
		if (destroyed) return;
