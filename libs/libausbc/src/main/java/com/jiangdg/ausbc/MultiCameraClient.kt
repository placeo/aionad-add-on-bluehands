...
class MultiCameraClient(ctx: Context, callback: IDeviceConnectCallBack?) {
        ...
        mUsbMonitor = USBMonitor(ctx, object : USBMonitor.OnDeviceConnectListener {
            ...
            override fun onDetach(device: UsbDevice?) {
                ...
                if (Utils.debugCamera) {
                    Timber.i("detach device name/pid/vid:${device?.deviceName}&${device?.productId}&${device?.vendorId} ")
                }
                device ?: return
            ...
            override fun onConnect(
                ...
                Timber.d("${this.javaClass.simpleName}, ${Throwable().stackTrace[0].methodName} called")
                if (Utils.debugCamera) {
                    Timber.i("connect device name/pid/vid:${device?.deviceName}&${device?.productId}&${device?.vendorId} ")
                }
                device ?: return
                if (!isUsbCamera(device) && !isFilterDevice(ctx, device)) {
                    return
            ...
            override fun onDisconnect(device: UsbDevice?, ctrlBlock: USBMonitor.UsbControlBlock?) {
                Timber.d("${this.javaClass.simpleName}, ${Throwable().stackTrace[0].methodName} called")
                if (Utils.debugCamera) {
                    Timber.i("disconnect device name/pid/vid:${device?.deviceName}&${device?.productId}&${device?.vendorId} ")
                }
                device ?: return
                if (!isUsbCamera(device) && !isFilterDevice(ctx, device)) {
                    return
            ...
            override fun onCancel(device: UsbDevice?) {
                Timber.d("${this.javaClass.simpleName}, ${Throwable().stackTrace[0].methodName} called")
                if (Utils.debugCamera) {
                    Timber.i("cancel device name/pid/vid:${device?.deviceName}&${device?.productId}&${device?.vendorId} ")
                }
                device ?: return
                if (!isUsbCamera(device) && !isFilterDevice(ctx, device)) {
                    return
    ...
    fun requestPermission(device: UsbDevice?): Boolean {
        Timber.d("${this.javaClass.simpleName}, ${Throwable().stackTrace[0].methodName} called")
        if (!isMonitorRegistered()) {
            Timber.w("Usb monitor haven't been registered.")
            return false
        }
    ...
    abstract class ICamera(val ctx: Context, val device: UsbDevice): Handler.Callback {
            ...
            when (msg.what) {
                MSG_START_PREVIEW -> {
                    ...
                    when (val cameraView = mCameraView) {
                        ...
                        Timber.i("surface measure size $measureSize")
        ...
        fun updateResolution(width: Int, height: Int) {
            if (mCameraRequest == null) {
                Timber.w("updateResolution failed, please open camera first.")
                return
            }
            mCameraRequest?.apply {
                ...
                Timber.i("updateResolution: width = $width, height = $height")
                closeCamera()
