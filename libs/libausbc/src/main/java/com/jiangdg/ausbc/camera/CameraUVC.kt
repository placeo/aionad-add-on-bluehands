...
import com.skt.photobox.DemoApplication
...
class CameraUVC(ctx: Context, device: UsbDevice) : MultiCameraClient.ICamera(ctx, device) {
    ...
    override fun getAllPreviewSizes(aspectRatio: Double?): MutableList<PreviewSize> {
        ...
        if (isMjpegFormat && (mUvcCamera?.supportedSizeList2?.isNotEmpty() == true)) {
            ...
            if (Utils.debugCamera) {
                Timber.i("aspect ratio = $aspectRatio, supportedSizeList = $sizeList")
            }
            mCameraPreviewSize
        }?.onEach { size ->
            val width = size.width
    ...
    override fun <T> openCameraInternal(cameraView: T) {
        if (Utils.isTargetSdkOverP(ctx) && !CameraUtils.hasCameraPermission(ctx)) {
            ...
            Timber.e("open camera failed, need Manifest.permission.CAMERA permission when targetSdk>=28")
            return
        }
        if (mCtrlBlock == null) {
            closeCamera()
        ...
        // YKK_TEST 2025-07-07 set preview size to 1280x720
        // YKK_TEST 2025-07-21 set preview size as global resolution
        Timber.i("resolution is set as ${request.previewWidth}x${request.previewHeight}")
        var previewSize = getSuitableSize(request.previewWidth, request.previewHeight).apply {
            mCameraRequest!!.previewWidth = this.width
            mCameraRequest!!.previewHeight = this.height
        }
        ...
        try {
            Timber.i("getSuitableSize: $previewSize")
            if (! isPreviewSizeSupported(previewSize)) {
                closeCamera()
                postStateEvent(ICameraStateCallBack.State.ERROR, "unsupported preview size")
                Timber.e("open camera failed, preview size($previewSize) unsupported-> ${mUvcCamera?.supportedSizeList2}")
                return
            }
            ...
            try {
                ...
                Timber.e(" setPreviewSize failed(format is $previewFormat), try to use other format...")
                mUvcCamera?.setPreviewSize(
                    previewSize.width,
                    previewSize.height,
                    DemoApplication.getGlobalFps(),
                ...
                Timber.e(" setPreviewSize failed, even using yuv format", e)
                return
            }
        }
        ...
        if (Utils.debugCamera) {
            Timber.i(" start preview, name = ${device.deviceName}, preview=$previewSize")
        }
    }
...
//        releaseEncodeProcessor()
        ...
        mUvcCamera = null
        if (Utils.debugCamera) {
            Timber.i(" stop preview, name = ${device.deviceName}")
        }
    }
