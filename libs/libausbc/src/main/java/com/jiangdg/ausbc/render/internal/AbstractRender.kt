...
abstract class AbstractRender(context: Context) {
    ...
    fun initGLES() {
        ...
        if (mProgram == 0) {
            Timber.e("create program failed, err = ${GLES20.glGetError()}")
            return
        }
        ...
        if (isGLESStatusError()) {
            Timber.e("create external texture failed, err = ${GLES20.glGetError()}")
            return
        }
        init()
        Timber.i("init surface texture render success!")
    }
    ...
    fun releaseGLES() {
        ...
        clear()
        Timber.i("release surface texture render success!")
    }
    ...
    private fun loadShader(shaderType: Int, source: String): Int {
        ...
        if (compiled[0] == 0) {
            Timber.e("Could not compile shader, info = ${GLES20.glGetShaderInfoLog(shader)}, T = ${Thread.currentThread().name}")
    ...
    private fun createProgram(vertexSource: String, fragmentSource: String): Int {
        ...
        if (mVertexShader == 0) {
            Timber.i("vertexSource err = ${GLES20.glGetError()}: \n $vertexSource")
            return 0
        }
        mFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER, fragmentSource)
        if (mFragmentShader == 0) {
            Timber.i("fragmentSource err = ${GLES20.glGetError()}: \n $fragmentSource")
            return 0
        }
        // 创建链接程序，并将着色器依附到程序
    ...
    protected fun createTexture(textures: IntArray) {
        ...
        Timber.i("create texture, id = ${textures[0]}")
    }
    ...
    fun createOESTexture(): Int {
        ...
        Timber.i("create external texture, id = ${textures[0]}")
        return textures[0]
    }
