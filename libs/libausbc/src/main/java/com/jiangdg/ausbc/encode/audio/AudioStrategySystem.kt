...
class AudioStrategySystem : IAudioStrategy {
    ...
    override fun initAudioRecord() {
        try {
            ...
            if (Utils.debugCamera) {
                Timber.i( "initAudioRecord success")
            }
        } catch (e: Exception) {
            Timber.e( "initAudioRecord failed, err = ${e.localizedMessage}", e)
        }
    }
    ...
    override fun startRecording() {
        try {
            ...
            if (Utils.debugCamera) {
                Timber.i( "startRecording success-->${mAudioRecord?.recordingState}")
            }
        } catch (e: Exception) {
            Timber.e( "startRecording failed, err = ${e.localizedMessage}", e)
        }
    }
    ...
    override fun releaseAudioRecord() {
        try {
            ...
            mAudioRecord = null
            if (Utils.debugCamera) {
                Timber.i( "releaseAudioRecord success.")
            }
        } catch (e: Exception) {
            Timber.e( "releaseAudioRecord failed, err = ${e.localizedMessage}", e)
        }
    }
