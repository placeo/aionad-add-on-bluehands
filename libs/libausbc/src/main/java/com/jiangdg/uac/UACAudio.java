...
public class UACAudio {
    ...
    public void init(USBMonitor.UsbControlBlock ctrlBlock) {
        ...
        try {
            ...
            Timber.w("initAudio: err = " + e.getMessage());
        }
        mStatus = result<0 ? AudioStatus.ERROR : AudioStatus.CREATED;
        Timber.i("initAudio: " + result+ ", mNativePtr = " +mNativePtr);
    }

    public void startRecording() {
        if (mStatus == AudioStatus.RELEASED) {
            Timber.e("startRecording failed: init status error");
            return;
        }
        if (nativeStartRecord(mNativePtr) < 0) {
            Timber.e("startRecording: start failed");
            mStatus = AudioStatus.ERROR;
            return;
        }
        mStatus = AudioStatus.RUNNING;
        Timber.i("startRecording: success");
    }

    public void stopRecording() {
        if (mStatus != AudioStatus.RUNNING) {
            Timber.e("stopRecording: not in running");
            return;
        }
        if (nativeStopRecord(mNativePtr) < 0) {
            Timber.e("stopRecording: stop failed.");
            return;
        }
        mStatus = AudioStatus.STOPPED;
        Timber.i("stopRecording: success");
    }

    public void release() {
        nativeRelease(mNativePtr);
        mStatus = AudioStatus.RELEASED;
        Timber.i("release: success");
    }
