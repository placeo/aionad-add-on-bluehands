...
#include "uvchacks.h"
#include "gstreamer/CGstStreamSource.h"
...
void UVCCaptureBase::uvc_preview_frame_callback(uvc_frame_t *frame, void *vptr_args) {
    ...
    // Accumulate bytes for bitrate monitoring
    CMonitor::getInstance()->increaseCameraBytes(frame->data_bytes);
    ...
    if (LIKELY(pCaptureBase->isRunning())) {
        LOGV("format: %d, bytes: %d, width: %d, height: %d", frame->frame_format, frame->data_bytes, frame->width, frame->height);

        GstMapInfo writeBufferMap;
        GstBuffer* streamBuffer = nullptr;
        int streamBufferSize = frame->data_bytes;

        if(streamBufferSize > 0) {
            streamBuffer = gst_buffer_new_allocate(NULL, streamBufferSize, NULL);
        }
        else if(streamBufferSize <= 0) {
            LOGE("buffer allocation failed, the length is under 0, stream buffer size : %u, timestamp : %lld", streamBufferSize, std::chrono::duration_cast<std::chrono::nanoseconds>(timestamp.time_since_epoch()).count());
            return;
        }

        if(streamBuffer != nullptr) {
            GstClockTime adjustedFrameTimestamp = 0;
            pCaptureBase->frameCount_++;
            if (pCaptureBase->frameCount_ == 1) {
                pCaptureBase->isFirstFrame_ = true;
            } else {
                pCaptureBase->isFirstFrame_ = false;
            }

            struct timeval tv;
            gettimeofday(&tv, NULL);
            GstClockTime cameraStreamTimestamp = tv.tv_sec * GST_SECOND + tv.tv_usec * 1000;

            if(true == pCaptureBase->isFirstFrame_) {
                LOGI("first frame");
                adjustedFrameTimestamp = 0;
                pCaptureBase->frameTimestampOffset_ = cameraStreamTimestamp;
            } else {
                adjustedFrameTimestamp = cameraStreamTimestamp - pCaptureBase->frameTimestampOffset_;
            }

            gst_buffer_map(streamBuffer, &writeBufferMap, GST_MAP_WRITE);
            memcpy(writeBufferMap.data, frame->data, streamBufferSize);


            GST_BUFFER_PTS(streamBuffer) = adjustedFrameTimestamp;
            GST_BUFFER_DTS(streamBuffer) = adjustedFrameTimestamp;

            gst_buffer_unmap(streamBuffer, &writeBufferMap);

            gchar *ptsString = nullptr;
            gchar *dtsString = nullptr;

            ptsString = g_strdup_printf("%" GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_PTS(streamBuffer)));
            dtsString = g_strdup_printf("%" GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_DTS(streamBuffer)));
        
            LOGV("Pts : %s Dts : %s", ptsString, dtsString);
        
            g_free(ptsString);
            g_free(dtsString);
            
            // YKK_TEST 20250821 - 이펙트 전환 중에는 스트리밍 차단
            if (CGstStreamSource::getInstance()->getStreamingAllowedState()) {
                gst_app_src_push_buffer((GstAppSrc*)(CGstStreamSource::getInstance()->getAppSrc()), streamBuffer);
            } else {
                LOGV("Streaming blocked during effect transition, dropping frame");
                gst_buffer_unref(streamBuffer);
            }
        }
    }
